    ...
    <v-dialog v-model="dialogVisible" max-width="512">
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="deep-orange" variant="tonal" @click="addCollaborator"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-snackbar top color="error" v-model="errorMsg"> This Person doesn't exist </v-snackbar>
    <v-snackbar top color="error" v-model="sameEmail"> This Person doesn't exist </v-snackbar>
  </v-row>
</template>
  ...
  methods: {
    addCollaborator() {
      ...
      this.findPerson().then((status: User | null) => {
        ...
        backendService
          .post(
            '/api/project/addCollaborator',
            { project_id: this.project['_id'], user_id: status['id'] },
            false
          )
          .then(() => {
    ...
    async findPerson(): Promise<User | null> {
      try {
        ...
        return await backendService.get(
          '/api/user/userByEmail?email=' + this.collaboratorEmail,
          false
        )
      } catch (error) {
        return null
