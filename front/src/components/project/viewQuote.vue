<template>
  <v-row justify="center" style="margin: 0px">
    <v-dialog v-model="dialogVisible" max-width="1024">
              ...
              <v-col cols="12" sm="12" md="12">
                <div>
                  <v-expansion-panels expand-icon="mdi-chevron-down" hide-actions>
                    <v-expansion-panel v-for="(quote, index) in quote_list" :key="index">
...
export default {
  ...
  methods: {
    getProductInfo() {
      backendService
        .get('/api/product/productById?id=' + this.productId, false)
        .then((response: any) => {
