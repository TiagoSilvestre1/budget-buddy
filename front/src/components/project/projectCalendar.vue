...
export default {
  ...
  methods: {
    ...
    {
        let arr: any[] = [];
  const response: API = await backendService.get('api/project/byUserId?user_id=' + this.getUser.id);
  if ('success' in response && response.success === true) {
    ...
    for (const project of this.projects) {
      ...
      for (const prod_id of project.products) {
        const res: AxiosResponse = await backendService.get('api/product/productById?id=' + prod_id, true);
        if (res.status === HttpStatusCode.Ok) {
          let product: Product = res.data;

          for (const el of product.quotes) {
            const quote_res: AxiosResponse = await backendService.get('api/product/quoteById?id=' + el, true);
